...
GENESET_CELL=/data/deyk/kushal/Miao_DO/data/MAGMA/SETS_MIAO_propQTL
...
do
  ...
  then
    $MAGMA_CELL/magma --gene-results $RAW_CELL/$traitname.gene_trait.genes.raw --set-annot $SET_CELL/miao_propQTL_celltypes.set --out $GENESET_CELL/$traitname
  fi
done
