...
pdf(paste0(my_path, "results/figures/Figure-1F-gviz-genemodel-calca.pdf"))
...
Muc2_expression <- fread(paste0(data_path, "expression/Gene-list-C-expression.csv"),
                         select = c("V1", "Calca"))
...
p1 <- plotdf %>% 
  ggplot(aes_string("X_umap1", "X_umap2", color = "Calca")) + 
  geom_point(shape = 46) +
  scale_color_gradient(low = "lightgrey", high="darkblue") +
  theme_void() +
  labs(title = "Calca")

ggsave(filename = paste0(results_path, "figures/Figure-1F-umap-all-cells-Calca.pdf"),
       plot = p1,
       width = 7,
       height = 5,
       dpi = 330)
