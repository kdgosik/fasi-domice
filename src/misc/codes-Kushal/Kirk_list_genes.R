...
for(numc in 1:length(celltypes)){
  ...
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET1_HARBOR_eQTL_", celltypes[numc], ".txt"),
...
for(numc in 1:length(celltypes)){
  ...
  genes_human = ortholog_tabb$HGNC.symbol[match(genes, ortholog_tabb$MGI.symbol)]
  genes_human = genes_human[!is.na(genes_human)]
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET1_HARBOR_eQTL_expressed_", celltypes[numc], ".txt"),
              row.names = F, col.names = F, sep = "\t", quote=F)
  cat("We are at cell type:", numc, "\n")
}
...
for(numc in 1:length(celltypes)){
  ...
  genes_human = ortholog_tabb$HGNC.symbol[match(genes, ortholog_tabb$MGI.symbol)]
  genes_human = genes_human[!is.na(genes_human)]
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET2_HARBOR_eQTL_spec_expressed_", celltypes[numc], ".txt"),
              row.names = F, col.names = F, sep = "\t", quote=F)
  cat("We are at cell type:", numc, "\n")
}
...
for(numc in 1:length(celltypes)){
  ...
  genes_human = ortholog_tabb$HGNC.symbol[match(genes, ortholog_tabb$MGI.symbol)]
  genes_human = genes_human[!is.na(genes_human)]
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET3_HARBOR_eQTL_notexpressed_", celltypes[numc], ".txt"),
              row.names = F, col.names = F, sep = "\t", quote=F)
  cat("We are at cell type:", numc, "\n")
}
...
for(numc in 1:length(celltypes)){
  ...
  genes_human = ortholog_tabb$HGNC.symbol[match(genes, ortholog_tabb$MGI.symbol)]
  genes_human = genes_human[!is.na(genes_human)]
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET3_HARBOR_eQTL_zeroexpressed_", celltypes[numc], ".txt"),
              row.names = F, col.names = F, sep = "\t", quote=F)
  cat("We are at cell type:", numc, "\n")
}
...
for(numc in 1:length(celltypes)){
  ...
  genes_human = ortholog_tabb$HGNC.symbol[match(genes, ortholog_tabb$MGI.symbol)]
  genes_human = genes_human[!is.na(genes_human)]
  write.table(genes_human, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO/GENESET4_HARBOR_eQTL_notexpressed_inanyILC_",
                                         celltypes[numc],  ".txt"),
              row.names = F, col.names = F, sep = "\t", quote=F)
  cat("We are at cell type:", numc, "\n")
}
...
ll = list.files("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO")
for(numl in 1:length(ll)){
  xx = data.frame(fread(paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO", "/", ll[numl]), header=F))[,1]
  outdff = cbind.data.frame(xx, 1)
  write.table(outdff, file = paste0("/data/deyk/kushal/Miao_DO/data/Gene_Scores/Miao_DO", "/",
                                    ll[numl]),
              row.names = F, col.names = F, sep = "\t", quote=F)
}
