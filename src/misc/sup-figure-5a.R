...
# ## TODO:
#   - zoom in for Bcl2 for IL-5
#   - zoom in IL-22 Loci genes (Klf7?)
#   - others for IL-4 cytokine 

## Read marker map
marker_map <- readRDS(paste0(geno_path, "Regev_map_20171221.rds"))

cytokines <- fread(paste0(my_path, "results/qtl-steady-cytokines-lods.csv.gz"),
                   data.table = FALSE)
...
for( col in plot_cytokines ) {
  ...
  ggsave(filename = paste0(figure_path, "Supp.Fig.5A", i, "-qtl-cytokine-steady-", col, "-gwa.pdf"),
         plot = p1,
         height = 7,
         width = 7,
         dpi = 330)
